...
def count_parameters(m, x, y):
    ...
    for p in m.parameters():
        total_params += torch.FloatTensor([p.numel()])
    m.total_params[0] = float(counter_parameters(m.parameters()))
...
def count_convNd_ver2(m, x, y):
    ...
    # m.total_ops += torch.FloatTensor([int(output_size * kernel_ops)])
...
def count_adap_avgpool(m, x, y):
    kernel = torch.div(torch.FloatTensor([i for i in (x[0].shape[2:])]),
                       torch.FloatTensor([i for i in (y.shape[2:])]))
