...
def profile_origin(model, inputs, custom_ops=None, verbose=True, report_missing=False):
    ...
    def add_hooks(m):
        ...
        for p in m.parameters():
            m.total_params += torch.FloatTensor([p.numel()])
...
def profile(model, inputs, custom_ops=None, verbose=True, ret_layer_info=False, report_missing=False):
    handler_collection = {}
    types_collection = set()
    if custom_ops is None:
        custom_ops = {}
    ...
    def add_hooks(m):
        ...
        if m_type in custom_ops:  # if defined both op maps, use custom_ops to overwrite.
            fn = custom_ops[m_type]
            if m_type not in types_collection and verbose:
                print("[INFO] Customize rule %s() %s." % (fn.__name__, m_type))
        elif m_type in register_hooks:
            fn = register_hooks[m_type]
            if m_type not in types_collection and verbose:
                print("[INFO] Register %s() for %s." % (fn.__name__, m_type))
        else:
            if m_type not in types_collection and report_missing:
                prRed("[WARN] Cannot find rule for %s. Treat it as zero Macs and zero Params." % m_type)
    ...
    def dfs_count(module, prefix="\t"):
        total_ops, total_params = module.total_ops[0], 0
        ret_dict = {}
        for n, m in module.named_children():
            ...
            if m in handler_collection and not isinstance(m, (nn.Sequential, nn.ModuleList)):
                m_ops, m_params = m.total_ops[0], m.total_params[0]
            else:
